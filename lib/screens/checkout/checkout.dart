...
class _CheckoutState extends State<Checkout> {
  ...
  String balance() {
    ...
    // تنظيف السلسلة من أي رموز غير رقمية
    balance = balance!.replaceAll(RegExp(r'[^\d.]'), '');
  ...
  fetchSummary() async {
    ...
    if (cartSummaryResponse != null) {
      ...
      // إزالة رموز العملة أو أي نصوص غير رقمية من grand_total قبل تحويلها إلى double
      String grandTotalCleaned =
          cartSummaryResponse.grand_total?.replaceAll(RegExp(r'[^\d.]'), '') ??
              "0.0";
      _grandTotalValue = double.tryParse(grandTotalCleaned) ?? 0.0;

      _totalString =
          cartSummaryResponse.grand_total; // الاحتفاظ بالنص الأصلي لعرضه
                      ...
                      child: Padding(
                        ...
                        child:
                          ...
                          fit: BoxFit.fitWidth,
                            ...
                            CachedNetworkImage(
                          imageUrl: _paymentTypeList[index].payment_type ==
                                  "manual_payment"
                              ? _paymentTypeList[index].image
                              : _paymentTypeList[index].image,
  ...
  BottomAppBar buildBottomAppBar(BuildContext context) {
    return BottomAppBar(
      child: Container(
        ...
        height: 60, // يمكنك زيادة الارتفاع قليلاً ليبدو أكثر راحة
        padding: EdgeInsets.symmetric(vertical: 4), // إضافة بعض الهوامش
        child: Row(
          mainAxisAlignment: MainAxisAlignment
              .center, // استخدام "center" لتكون الأزرار متناسقة
          children: [
            Btn.minWidthFixHeight(
              minWidth: MediaQuery.of(context).size.width *
                  0.9, // تصغير الزر قليلاً لترك مساحة حوله
              height: 50,
              color: MyTheme.accent_color,
              shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(
                    8.0), // إضافة بعض الاستدارة للزر لتنعيم الحواف
              ),
              child: Text(
                widget.paymentFor == PaymentFor.WalletRecharge
